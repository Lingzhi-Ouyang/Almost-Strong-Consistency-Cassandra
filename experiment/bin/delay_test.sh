...
delays=$(grep '^server_delay_in_ms_list =' $file | sed 's/^server_delay_in_ms_list = \[\(.*\)\]/\1/g')
delays=(${delays//,/ })
echo ${delays[@]}
...
for delay in ${delays[@]}; do
    ...
    delay=$(echo $delay | tr -cd "[0-9]")
    jitter=` expr $delay / 2 `
    echo $jitter

    if [[ $delay -eq $default_inter_delay && -n $nodefault ]]; then
        continue
    fi
    ...
    ./cassandra.sh $cluster $snitch $digest $repair $default_intra_delay $default_intra_jitter $delay $jitter
    sleep 5
...
echo "  -Setting up tuning info in conf.py"
file="conf.py"
sed -i -e "s|^parameter_name_list =.*|parameter_name_list = \[\"server_delay_in_ms\"\]|g" $file
sed -i -e "s|^default_server_delay_in_ms =.*|default_server_delay_in_ms = '$default_inter_delay'|g" $file
